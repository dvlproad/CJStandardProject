...
#import "CJDemoAlert.h"
#import "CJDemoButtonFactory.h"
#import <CJBaseUIKit/UIButton+CJMoreProperty.h>

@interface CJDemoAlert () {
...
@implementation CJDemoAlert

+ (CJDemoAlert *)sharedInstance {
    static CJDemoAlert *_sharedInstance = nil;
...
+ (void)showIKnowAlertViewWithTitle:(NSString *)title {
    [CJDemoAlert showAlertViewWithTitle:title
...
+ (void)showNetworkNoOpenAlert:(BOOL)show {
    if (!show) {
        if ([CJDemoAlert sharedInstance].networkNoOpenAlert) {
            [[CJDemoAlert sharedInstance].networkNoOpenAlert dismissWithDelay:0];
            [CJDemoAlert sharedInstance].networkNoOpenAlert = nil;
        }
    } else {
        if ([CJDemoAlert sharedInstance].networkNoOpenAlert) {
            return;
        }
        [CJDemoAlert sharedInstance].networkNoOpenAlert = [self cjDemoAlertViewWithTitle:NSLocalizedString(@"网络链接失败，请检查您的网络链接", nil) okButtonTitle:NSLocalizedString(@"查看设置", nil) okHandle:^{
            [CJDemoAlert sharedInstance].networkNoOpenAlert = nil;
            //打开系统设置
            
        }];
        UIColor *blankBGColor = [UIColor colorWithRed:.16 green:.17 blue:.21 alpha:.6];
        [[CJDemoAlert sharedInstance].networkNoOpenAlert showWithShouldFitHeight:YES blankBGColor:blankBGColor];
    }
}
...
+ (void)showLocationNoOpenAlert:(BOOL)show {
    if (!show) {
        if ([CJDemoAlert sharedInstance].locationNoOpenAlert) {
            [[CJDemoAlert sharedInstance].locationNoOpenAlert dismissWithDelay:0];
            [CJDemoAlert sharedInstance].locationNoOpenAlert = nil;
        }
    } else {
        ...
        [CJDemoAlert sharedInstance].locationNoOpenAlert = [self cjDemoAlertViewWithTitle:NSLocalizedString(@"您没开启GPS，无法接单", nil) okButtonTitle:NSLocalizedString(@"去开启", nil) okHandle:^{
            [CJDemoAlert sharedInstance].locationNoOpenAlert = nil;
            //打开系统设置
            
        }];
        UIColor *blankBGColor = [UIColor colorWithRed:.16 green:.17 blue:.21 alpha:.6];
        [[CJDemoAlert sharedInstance].locationNoOpenAlert showWithShouldFitHeight:YES blankBGColor:blankBGColor];
    }
}
...
+ (void)showLocationAbnormalAlert:(BOOL)show {
    if (!show) {
        if ([CJDemoAlert sharedInstance].locationAbnormalAlert) {
            [[CJDemoAlert sharedInstance].locationAbnormalAlert dismissWithDelay:0];
            [CJDemoAlert sharedInstance].locationAbnormalAlert = nil;
        }
    } else {
        ...
        [CJDemoAlert sharedInstance].locationAbnormalAlert = [self cjDemoAlertViewWithTitle:NSLocalizedString(@"获取定位权限异常，请手动授权APP定位权限", nil) okButtonTitle:NSLocalizedString(@"我知道了", nil) okHandle:^{
            [CJDemoAlert sharedInstance].locationAbnormalAlert = nil;
            //打开系统设置
            
        }];
        UIColor *blankBGColor = [UIColor colorWithRed:.16 green:.17 blue:.21 alpha:.6];
        [[CJDemoAlert sharedInstance].locationAbnormalAlert showWithShouldFitHeight:YES blankBGColor:blankBGColor];
    }
}
                                 ...
                                 okHandle:(void(^)(void))okHandle
{
    ...
    //bottomButtons
    UIButton *okButton = [CJDemoButtonFactory blueButton];
                      ...
                      okHandle:(void(^)(void))okHandle
{
    ...
    //bottomButtons
    UIButton *cancelButton = [CJDemoButtonFactory whiteButton];
