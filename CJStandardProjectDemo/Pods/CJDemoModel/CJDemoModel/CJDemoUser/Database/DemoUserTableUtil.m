...
static NSString *kCurrentTableName = @"DemoUser";


@implementation DemoUserTableUtil
...
+ (DemoUser *)selectAccountInfoWhereUID:(NSString *)uid {
    ...
    NSError *error;
    NSArray<DemoUser *> *users = [MTLJSONAdapter modelsOfClass:[DemoUser class] fromJSONArray:result error:&error];
    if (error) {
        //NSAssert(NO, @"类型转换发生错误，请检查");
        NSLog(@"类型转换发生错误，请检查");
    }
//    NSArray<DemoUser *> *users = [DemoUser mj_objectArrayWithKeyValuesArray:result];
